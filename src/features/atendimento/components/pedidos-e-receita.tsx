...
export function PedidosEReceita({ tiposExame, pedidos, medicamentos, observacoes, onPedidosChange, onMedicamentosChange, onObservacoesChange }: {
  ...
  return <div className="space-y-5 border-t pt-5">
    ...
    {medicamentos.length > 0 && <textarea className="min-h-[60px] w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm" placeholder="Observações gerais da receita" value={observacoes} onChange={(event) => onObservacoesChange(event.target.value)} />}
  </div>;
}
